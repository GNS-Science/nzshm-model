...
def test_source_logic_tree_uncertainty_puy():
    ...
    assert src_logic_tree.branch_sets[0].branchSetID == "PUY"
    assert src_logic_tree.branch_sets[0].branches[0].path() == PurePath(
        '[dm0.7, bN[0.902, 4.6], C4.0, s0.28]', 'PUY', 'NSHM_v1.0.4'
    )

    assert len(src_logic_tree.branch_sets[0].branches[0].uncertainty_models) == 2
    ...
    assert src_logic_tree.branch_sets[0].branches[0].uncertainty_models[0] == "SW52ZXJzaW9uU29sdXRpb25Ocm1sOjEyOTE2NTc="
    ...
    assert src_logic_tree.branch_sets[0].branches[0].uncertainty_models[1] == "RmlsZToxMzA3NTM="
...
def test_source_logic_tree_uncertainty_slab():
    ...
    assert src_logic_tree.branch_sets[3].branchSetID == "SLAB"
    assert src_logic_tree.branch_sets[3].branches[0].path() == PurePath('[runiform, d1]', 'SLAB', 'NSHM_v1.0.4')

    assert len(src_logic_tree.branch_sets[3].branches[0].uncertainty_models) == 1
    ...
    assert src_logic_tree.branch_sets[3].branches[0].uncertainty_models[0] == "RmlsZToxMjEwMzM="
