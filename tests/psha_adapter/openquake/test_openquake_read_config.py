...
# @pytest.mark.parametrize("config", [config_site_specific, config_uniform_sites])
# def test_read_oqini_roundtrip_uniform_site(tmp_path, config):

#     adapter: OpenquakeConfigPshaAdapter = config_uniform_sites.psha_adapter(OpenquakeConfigPshaAdapter)
#     ini_filepath = adapter.write_config(tmp_path)
#     config_from_ini: OpenquakeConfig = adapter.read_config(ini_filepath)

#     # which equality should be used?
#     assert config_from_ini.compatible_hash_digest() == config_uniform_sites.compatible_hash_digest()
#     assert config_from_ini == config_uniform_sites
